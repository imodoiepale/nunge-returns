...
class AnalyticsService {
  ...
  async getTrustedByCount() {
    try {
      // Get the count from the user_metrics table
      ...
      // Return the latest total_users count or a fallback value
      return data?.total_users || 5000;
    } catch (error) {
      console.error('Error getting trusted by count:', error);
  ...
  _formatReturnMetrics(returnMetrics) {
    ...
    returnMetrics.forEach(item => {
      ...
      dateMap[item.date][item.status] = parseInt(item.count);
    });
