...
async function navigateToFileReturn(page, company) {
    ...
    const receiptFileName = `${company.company_name} - ${company.kra_pin} - NIL RETURN - ACKNOWLEDGEMENT RECEIPT - ${date}.pdf`;
    ...
    try {
        ...
        const storageFileName = `receipts/company_${company.kra_pin.replace(/\//g, '-')}_${date}.pdf`;
        ...
        if (uploadError) {
            ...
            if (urlData && urlData.publicUrl) {
                ...
                return { localPath: receiptFilePath, storageUrl: receiptUrl };
            }
        }
    } catch (storageError) {
    ...
    return { localPath: receiptFilePath, storageUrl: null };
}
    ...
    try {
        ...
        await supabase
            ...
            .insert([{
                ...
                description: `Company nil return filing ${status === 'Valid' ? 'completed' : 'failed'}`,
                metadata: {
                    status,
...
export async function POST(req) {
    try {
        ...
        // Log that we're processing this company
...
console.log(`- Download path: ${downloadFolderPath}`);
        ...
        try {
            ...
            const row = worksheet.addRow([null, company.company_name, company.kra_pin, "********", status]);
        ...
        if (session_id && return_id) {
            await updateSupabaseStatus(session_id, return_id, status, receiptPaths.storageUrl);

            // Create a document record if we have a receipt URL
            if (receiptPaths.storageUrl && receiptPaths.localPath) {
                try {
                    ...
                    await supabase
                        ...
                        .insert([{
                            ...
                            document_url: receiptPaths.storageUrl,
        ...
        return NextResponse.json(
            { error: "Failed to process company nil return" },
            { status: 500 }
        );
    }
}
